...
export async function connectDashboardTestDb(input: { dbName?: string } = {}): Promise<string> {
...
export async function resetDashboardTestDb(): Promise<void> {
  if (mongoose.connection.readyState === 0) {
    return;
  }
...
export async function disconnectDashboardTestDb(): Promise<void> {
  if (mongoose.connection.readyState !== 0) {
    await mongoose.disconnect();
  }
