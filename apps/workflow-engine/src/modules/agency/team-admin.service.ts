...
import { canManageAgencyTeam, canManageTargetAgency, isPlatformAdmin } from './authorization.js';
...
function createInvitationExpiryDate(): Date {
  return new Date(Date.now() + INVITATION_DURATION_MS);
}

function buildInvitationSignupPath(authBasePath: string | undefined, invitationToken: string): string {
  const normalizedBasePath = (authBasePath && authBasePath.trim().length > 0 ? authBasePath : '/auth')
    .replace(/\/+$/, '');
  const signupPath = `${normalizedBasePath}/signup`;
...
export async function getAgencyUserDetail(session: SessionRecord, agencyId: string, userId: string): Promise<TeamMemberRecord> {
...
export async function getAgencyInvitationDetail(
  session: SessionRecord,
  agencyId: string,
  invitationId: string,
): Promise<AgencyInvitationRecord> {
  await dbConnect();
  assertAgencyAccess(session, agencyId);

  const invitationObjectId = ensureInvitationIdInput(invitationId);
  const invitation = await AgencyInvitationModel.findOne({ _id: invitationObjectId, agencyId }).lean();
...
export async function updateAgencyUser(
  ...
  const defaultId = nextTenantIds.find((id) => id.equals(nextDefaultTenantId)) || nextTenantIds[0] || targetUser.defaultTenantId;
