...
describe('pairing route', () => {
  beforeAll(async () => {
    await connectWorkflowEngineTestDb({ dbName: 'noxivo-pairing-route-tests' });
  });
  ...
  it('requests raw QR payload from MessagingProvider and returns it as JSON', async () => {
    ...
    const binding = await MessagingSessionBindingModel.create({
      ...
      messagingSessionName: 'pairing-session',
      status: 'pending'
    });
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.endsWith('/api/pairing-session/auth/qr?format=raw')) {
        return new Response(JSON.stringify({ value: 'raw-qr-token' }), {
          status: 200,
    ...
    try {
      const response = await server.inject({
        method: 'GET',
        url: `/api/v1/sessions/${binding._id.toString()}/qr`,
        headers: {
