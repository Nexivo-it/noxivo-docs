...
│  │  - Manage all MessagingProvider sessions                                     │   │
...
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  Connected to MessagingProvider (WhatsApp)                                   │   │
│  └─────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────┘
           │                                         │
...
The engine looks up the dashboard by API key and associates the request with that agency.

### 3. Webhook Routing

When MessagingProvider sends webhooks (new messages, status changes, etc.), the engine:
1. Identifies which agency/tenant the event belongs to
2. Looks up the dashboard's webhook URL from `DashboardConfig`
3. Forwards the event to the correct dashboard with the webhook secret

### 4. Engine Admin Dashboard

The engine's own admin dashboard shows:
- All registered dashboards
- All agencies across all dashboards
- All MessagingProvider sessions with agency/tenant metadata
...
interface DashboardConfig {
  agencyId: string;           // Unique ID from dashboard
  dashboardName: string;     // Display name
  dashboardUrl: string;      // Base URL for webhooks
  webhookSecret: string;     // Secret for webhook validation
  apiKey: string;            // Engine API key for this dashboard
...
| POST | `/api/v1/admin/dashboards/:agencyId/sync` | Mark as synced |
...
# Optional - for webhook forwarding
MessagingProvider_BASE_URL=https://messaging.example.com
MessagingProvider_API_KEY=...
```
...
## Session & Agency Metadata

All MessagingProvider session bindings include metadata that identifies the owning dashboard:

```javascript
{
  agencyId: "agency_abc123",      // From dashboard
  tenantId: "tenant_xyz789",     // From dashboard
  clusterId: "cluster_1",
  messagingSessionName: "wa_agency_abc123_tenant_xyz789",
  status: "WORKING"
}
```

This allows the engine admin to display all sessions with their dashboard association.
...
- Both dashboard and engine must agree on `agencyId`/`tenantId` format
- Engine stores agencyId as string - ensure dashboard uses consistent IDs
- Webhook retry logic should be implemented on dashboard side