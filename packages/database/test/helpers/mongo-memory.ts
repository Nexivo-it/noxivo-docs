...
export async function connectDatabaseTestDb(input: { dbName?: string } = {}): Promise<string> {
...
export async function resetDatabaseTestDb(): Promise<void> {
  if (mongoose.connection.readyState === 0) {
    return;
  }
...
export async function disconnectDatabaseTestDb(): Promise<void> {
  if (mongoose.connection.readyState !== 0) {
    await mongoose.disconnect();
  }
